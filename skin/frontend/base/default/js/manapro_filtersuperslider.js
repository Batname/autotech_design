...
var $JPS = jQuery.noConflict();
ManaPro.filterSuperSlider = function(id, o) {
    var _changing = false;
    var $_from = $JPS('#' + id + '-applied input.m-slider.m-from');
    var $_to = $JPS('#' + id + '-applied input.m-slider.m-to');
    var _value = [
    ...
    function _change(value, undefined) {
        ...
        if (value[0] <= o.rangeFrom && value[1] >= o.rangeTo) {
            window.setLocation($JPS.base64_decode(o.clearUrl));
        }
        else {
            var formattedValue = [_round(value[0]), _round(value[1])];
            window.setLocation($JPS.base64_decode(o.url).replace("__0__", formattedValue[0]).replace("__1__", formattedValue[1]));
        }
        _changing = false;
    }
    ...
    s.options.onSlide = function(value) {
        if (o.manualEntry) {
            $JPS('#'+id+'-applied input.m-slider.m-from').val(_round(value[0]));
            $JPS('#'+id+'-applied input.m-slider.m-to').val(_round(value[1]));
        }
        else {
    ...
    if (_mana_oldResizehandler[id]) {
        $JPS(window).unbind('resize', _mana_oldResizehandler[id]);
        _mana_oldResizehandler[id] = null;
    }
    _mana_oldResizehandler[id] = _resizeSpanAndHandles;
    $JPS(_resizeSpanAndHandles);

    $JPS(window).bind('resize', _resizeSpanAndHandles);
    $JPS(document).bind('m-ajax-after', _resizeSpanAndHandles);
    $JPS('#' + id + '-track').parent().on('m-prepare', function () {
        _resizeSpanAndHandles(true);
    });
    $JPS(document).on('click', '.toggle-content dl:first dt', function () {
        _resizeSpanAndHandles(true);
    });
    //$JPS('body').click(_resizeSpanAndHandles);
};
ManaPro.filterAttributeSlider = function (id, o) {
    ...
    function _change(value, undefined) {
        ...
        if (indexes[0] <= _indexOf(o.rangeFrom) && indexes[1] >= _indexOf(o.rangeTo)) {
            ...
            window.setLocation($JPS.base64_decode(o.url).replace("__0__", formattedValue));
        }
    }
