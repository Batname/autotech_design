...
class Sitemaster_Exportpriceua_IndexController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        $this->loadLayout();
        $this->_setActiveMenu('catalog/exportpriceua');
        $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
        $this->_addContent($this->getLayout()->createBlock('exportpriceua/list'));
        $this->renderLayout();
    }


    public function exportPostPriceUaAction()
    {

        //check for selected categories
        if(!isset($_POST['cats'])) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('exportpriceua')->__('Not selected Categories'));
            $this->_redirect('*/*');
            return;
        }
        ...
        //name of store
        $site_name = Mage::getStoreConfig('exportpriceua/settings/storename',Mage::app()->getStore());;
...
//		$url_->appendChild($url_name);
        ...
        $currency->setAttribute("code", Mage::app()->getStore()->getCurrentCurrencyCode());
        ...
        header('Content-Disposition:  attachment; filename="price-ua1.xml"');
