...
class Sitemaster_Checkout_CheckoutController extends Mage_Checkout_Controller_Action
{
    /**
    ...
    protected function _subscribeCustomer($customer, $isWantSubscribe)
    {
        ...
        $status = Mage::getModel('sitemaster_checkout/subscriber')
    ...
    protected function _saveAddressesFor($sessionOrder)
    {
        ...
        $address = Mage::helper('sitemaster_checkout')->extractAddressFrom($sessionOrder);
     ...
     *
     * @return Sitemaster_Checkout_Model_Page
     */
    public function getOnepage()
    {
        return Mage::getSingleton('sitemaster_checkout/page');
    }
    ...
    public function locationAutocompleteAction()
    {
        ...
        $location = Mage::helper('sitemaster_checkout')->explodeLocation($location);
        $items = Mage::getResourceModel('sitemaster_checkout/countries')->search($location);

        return $this->getResponse()->setBody($this->getLayout()
            ->createBlock('core/template')
            ->setTemplate('sitemaster/autocomplete.phtml')
            ->setItems($items)
            ->toHtml()
        );
    }
    ...
    public function updateAction()
    {
        ...
        $this->getLayout()->getUpdate()->load('sitemaster_checkout_update');
        $this->getLayout()->generateXml()->generateBlocks();

        if (isset($type['shipping'])) {
            $blocks['shipping'] = $this->getLayout()
                ->getBlock('sitemaster_checkout.shipping_method.available')
                ->toHtml();
        }

        if (isset($type['payment'])) {
            $blocks['payment'] = $this->getLayout()
                ->getBlock('sitemaster_checkout.payment_methods')
                ->toHtml();
        }
    ...
    public function verifyAction()
    {
        ...
        if ($verifyUrl) {
            $html = $this->getLayout()->createBlock('core/template')
                ->setTemplate('sitemaster/checkout/centinel/authentication.phtml')
                ->setFrameUrl($verifyUrl)
                ->toHtml();
        } else {
            $html = $this->getLayout()->createBlock('core/template')
                ->setTemplate('sitemaster/checkout/centinel/complete.phtml')
                ->setIsProcessed(true)
                ->setIsSuccess(true)
                ->toHtml();
        }
