...
class Sitemaster_Checkout_Block_Block_Info extends Mage_Checkout_Block_Onepage_Abstract
{
    /**
     ...
     *
     * @var Sitemaster_Checkout_Model_Order
     */
    protected $_order;
    ...
    {
        $this->_order = $this->helper('sitemaster_checkout')->buildSessionOrder();
        $this->setTemplate('sitemaster/checkout/block/info.phtml');
    ...
    protected function _collectCustomerLocation()
    {
        ...
        if (!$this->helper('sitemaster_checkout')->useOnlyDefaultCountry()) {
     ...
     *
     * @see Sitemaster_Checkout_Helper_Data::useOnlyDefaultCountry
     * @return string
     */
    ...
    public function getCountryBlock($type = 'shipping')
    {
        ...
        if (!$countryId) {
            $countryId = $this->helper('sitemaster_checkout')->getDefaultCountryId();
        }
        $select = $this->getLayout()->createBlock('core/html_select')
            ->setTitle($this->helper('checkout')->__('Country'))
            ->setClass('validate-select non-storable form-control')
            ->setValue($countryId)
