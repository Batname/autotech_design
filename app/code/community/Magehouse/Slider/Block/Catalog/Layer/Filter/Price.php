	...
	*/
	...
	public function setNewPrices(){
		...
		if(count($loadedCollection) > 0){
			$loadedMin = round($loadedCollection[0]['min_price']);
			$loadedMax = round($loadedCollection[$tot-1]['min_price']);
		}
		...
		if($this->isTextBoxEnabled()){
			$html = '
				<div class="slider-text-box">
					<p class="currency-symbol">'.$this->getCurrencySymbol().'</p> <input type="text" name="min" id="minPrice" class="priceTextBox" value="'.round($this->getCurrMinPrice()).'" style="'.$textBoxStyle.'" />
					<p class="currency-symbol">'.$this->getCurrencySymbol().'</p> <input type="text" name="max" id="maxPrice" class="priceTextBox" value="'.round($this->getCurrMaxPrice()).'" style="'.$textBoxStyle.'" />
					<button type="button" value="'.$this->getGoBtnText().'" name="go" class="go button go-button" style="width: 90%'.$goBtnStyle.'" >'.$this->getGoBtnText().'</button>
