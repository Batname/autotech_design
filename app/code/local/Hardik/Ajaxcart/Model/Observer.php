...
class Hardik_Ajaxcart_Model_Observer {
    ...
    public function addToCartEvent($observer) {
        ...
        if (!$request->getParam('in_cart') && !$request->getParam('is_checkout')) {
            ...
            $checkout_link = Mage::getUrl('checkout/cart');
            $full_massage = "<p>".Mage::helper('checkout')->__('%s was added into cart.', $observer->getProduct()->getName()) ."<p>";
            $checkout_massage =  Mage::helper('checkout')->__('Go to Checkout');
            $checkout_link =  "<button class='hardik-button first-hardik-button' onclick=\"setLocation('$checkout_link')\" title=".">".$checkout_massage."</button>";
            $back_link = "<button class='clouse-me hardik-button'>".Mage::helper('checkout')->__('Continue shopping')."</button>";
            $javascript = "<script>jQuery( \".clouse-me\" ).click(function() {
            jQuery( \".Growler-notice\" ).fadeOut(\"slow\");
            });
            </script>";

            $_response = Mage::getModel('ajaxcart/response')
                    ->setProductName($observer->getProduct()->getName())
                    ->setMessage($full_massage .$checkout_link. $back_link.$javascript);
