 ...
 * @package     Mage_Shell
 * @copyright   Copyright (c) 2012 JoomlaCreator Inc. (http://www.joomlacreator.com)
 ...
 */
class JoomlaCreator_Shell_Cache extends Mage_Shell_Abstract {
	/**
	...
	public function info() {
		...
		foreach ($this->_getCacheTypes() as $cache) {
			...
			if ($enabled == 'Enabled') {
				$invalid = (array_key_exists($cache, $invalidTypes)) ? 'Invalid'
						: 'Valid';
			} else {
				$invalid = 'N/A';
			}
...
$shell = new JoomlaCreator_Shell_Cache();
$shell->run();
